...
impl Optimizer {
    ...
    fn dce_stmt(&mut self, stmt: Stmt) -> Option<Stmt> {
        match stmt {
            ...
            Stmt::If(cond, body, elseifs, else_body) => {
                ...
                let (new_elseifs, promoted_else) = self.dce_elif_chain(elseifs, else_body);

                match &cond {
                    Expr::Bool(true) => {
                        // The if-body always runs; elif/else are unreachable.
                        self.dce += new_elseifs.len() + if promoted_else.is_some() { 1 } else { 0 };
    ...
    fn dce_elif_chain(
        ...
        Vec<ElseIf>,
        Option<(Expr, Vec<Stmt>, Vec<ElseIf>, Option<Vec<Stmt>>)>,
    ) {
        ...
        while let Some(elif) = remaining_iter.next() {
            match &elif.condition {
                ...
                Expr::Bool(true) => {
                    ...
                    return (surviving, Some((elif.condition, elif.body, vec![], None)));
                }
    ...
    fn fold_binop(&mut self, left: Expr, op: Op, right: Expr) -> Expr {
        ...
        // x * 1  →  x       1 * x  →  x
        if let Op::Mul = &op {
            if let Expr::Int(1) = right {
                return self.reduced(left);
            }
            if let Expr::Int(1) = left {
                return self.reduced(right);
            }
        }
        ...
        if let Op::And = &op {
            ...
            if let Expr::Bool(false) = right {
                return self.reduced(Expr::Bool(false));
            }
            if let Expr::Bool(true) = left {
                return self.reduced(right);
            }
            if let Expr::Bool(false) = left {
                return self.reduced(Expr::Bool(false));
            }
        }
        ...
        if let Op::Or = &op {
            ...
            if let Expr::Bool(true) = right {
                return self.reduced(Expr::Bool(true));
            }
            if let Expr::Bool(false) = left {
                return self.reduced(right);
            }
        ...
        if let Op::EqEq = &op {
            ...
            if let Expr::Bool(false) = right {
                return self.reduced(Expr::Not(Box::new(left)));
            }
            if let Expr::Bool(true) = left {
                return self.reduced(right);
            }
            if let Expr::Bool(false) = left {
                return self.reduced(Expr::Not(Box::new(right)));
            }
        }
